...
class PgMini (object):
    ...
    def computeStepsPosition(self,alpha=0.0,p0=[-0.01,-0.005],v=[1.0,0.1],x0=[[0,0] , [0,0]],LR=True):
        #const definitions
        g               = self.g     
        h               = self.h
        Nstep           = self.Nstep
        ...
        A_p1 = np.zeros([Nstep,Nstep-1])
        for i in range(Nstep):
            for j in range(0, i):
                A_p1[i,j]=(Fx**(i-j-1)*Fu)[1,0]
        A_p2_x = np.zeros([Nstep-1,Nstep-1])
        A_p2_y = np.zeros([Nstep-1,Nstep-1])
        A_p2_x[0,0]  = -beta_x
        A_p2_y[0,0]  = -beta_y
        ...
        A_p_x=np.vstack([A_p1,A_p2_x])
        A_p_y=np.vstack([A_p1,A_p2_y])
        ...
        for i in range(Nstep-1):
            b_p2_x[i]= beta_x *0
            b_p2_y[i]= beta_y*Dpy*(-1)**i

        b_p2_x[0]+= -beta_x*p0_x
        b_p2_y[0]+= -beta_y*p0_y

        b_p_x=np.vstack([b_p1_x,b_p2_x])
        b_p_y=np.vstack([b_p1_y,b_p2_y])
